...
impl<S: Signer, K: Kind> AuthenticationBuilder<'_, S, K> {
    ...
    pub async fn authenticate(self) -> Result<Client<Authenticated<K>>> {
        ...
        let auth_address = if matches!(self.signature_type, Some(SignatureType::Poly1271)) {
            funder.ok_or_else(|| {
                Error::validation(
                    "A deposit wallet funder address is required with a Poly1271 signature type",
                )
        ...
        let credentials = match self.credentials {
            ...
            None => {
                inner
                    .create_or_derive_api_key(self.signer, self.nonce, auth_address)
                    .await?
            }
        };
...
impl ClientInner<Unauthenticated> {
    pub async fn create_api_key<S: Signer>(
        ...
        let request = self
            ...
            .build()?;
        let headers = self.create_headers(signer, nonce, auth_address).await?;
    ...
    async fn create_or_derive_api_key<S: Signer>(
        ...
        match self.create_api_key(signer, nonce, auth_address).await {
            Ok(creds) => Ok(creds),
            Err(err) if err.kind() == ErrorKind::Status => {
                ...
                self.derive_api_key(signer, nonce, auth_address).await
            }
            Err(err) => Err(err),
        }
    }

    async fn create_headers<S: Signer>(
        &self,
        signer: &S,
        nonce: Option<u32>,
        auth_address: Address,
        ...
        let poly1271_funder = (auth_address != signer.address()).then_some(auth_address);
        auth::l1::create_headers(signer, chain_id, timestamp, nonce, poly1271_funder).await
    }
}
...
impl Client<Unauthenticated> {
    ...
    pub async fn create_api_key<S: Signer>(
        ...
        self.inner
            .create_api_key(signer, nonce, signer.address())
            .await
    }
    ...
    pub async fn derive_api_key<S: Signer>(
        ...
        self.inner
            .derive_api_key(signer, nonce, signer.address())
            .await
    }
    ...
    pub async fn create_or_derive_api_key<S: Signer>(
        ...
        self.inner
            .create_or_derive_api_key(signer, nonce, signer.address())
            .await
    }
}
