-- BTC five-minute realtime-paper experiment readiness audit.
...
WITH expected_migrations(name) AS (
  VALUES
    ...
    ('AllowUnstartedTradingProcesses1777124000000'::text)
)
...
SELECT
  ...
  AND coalesce((
    ...
    WHERE table_schema='polymarket'
      AND table_name='trading_processes'
      AND column_name='started_at'
  ),false) AS unstarted_trading_process_schema_gate_pass;
...
\echo '== selected cohort frozen runtime safety/config contract =='
WITH selected AS (
...
SELECT experiment_id, process_id,
       ...
       process_config #>> '{raw,preregistration_sha256}' AS preregistration_sha256,
       ...
       (
         ...
         AND
         process_config #>> '{raw,pipeline_version}' = 'btc_realtime_paper_pipeline_v11'
       ...
       ) AS frozen_runtime_safety_config_gate_pass
FROM selected;
...
WITH selected_experiment AS (
  SELECT e.experiment_id, e.process_id, e.config AS process_config
  FROM polymarket.btc_paper_experiments e
  WHERE e.experiment_id = NULLIF(:'experiment_id', '')::uuid
), scoped_orders AS (
  SELECT o.*,
...
SELECT count(o.order_id) AS paper_order_plans,
       ...
       (count(o.order_id) > 0 AND coalesce(sum(o.fill_rows), 0) > 0)
         AS paper_execution_non_vacuously_exercised,
...
FROM selected_experiment e
JOIN polymarket.fills f ON f.process_id = e.process_id
 AND f.created_at >= e.started_at
 AND f.created_at <= coalesce(e.stopped_at, now())
LEFT JOIN polymarket.orders o ON o.order_id = f.order_id
WHERE o.order_id IS NULL
   OR f.source <> 'paper'
   OR o.raw_payload #>> '{request,metadata,experiment_id}' IS DISTINCT FROM e.experiment_id::text
