-- Phase 6 deterministic BTC five-minute realtime-paper report.
--
...
-- docs/experiments/btc-5m-chainlink-fair-value-phase6-20260713-b/preregistration.json
-- owns the thresholds. scripts/btc-phase6-audit.sh reads that manifest and supplies
...
\if :{?phase6_experiment_key}
\else
  \set phase6_experiment_key btc-5m-chainlink-fair-value-phase6-20260713-b
\endif
\if :{?audit_mode}
\else
...
\else
  \set expected_compiled_source_identity tree-sha256:ea9d1ab51e0b4283eba67a48ade2b0ef177c8d60084c9f504f7d77ced85c80e7
\endif
...
\echo '== Phase 6 preregistered parameters =='
SELECT :'audit_mode' AS audit_mode,
       :'phase6_experiment_key' AS preregistered_experiment_key,
...
WITH selected AS (
  SELECT e.*, p.process_type, p.process_scope, p.process_key,
         ...
         p.heartbeat_at, p.config AS process_config, p.metadata AS process_metadata
...
SELECT experiment_id, name, status AS experiment_status, process_id,
       ...
       process_metadata->>'compiled_source_identity' AS compiled_source_identity,
       process_metadata->>'preregistration_sha256' AS recorded_preregistration_sha256,
       ...
       (
         name = :'phase6_experiment_key'
         AND process_key = :'phase6_experiment_key'
...
WITH selected AS (
  SELECT e.*, p.process_type, p.process_scope, p.process_key,
         p.config AS process_config, p.metadata AS process_metadata
  FROM polymarket.btc_paper_experiments e
  JOIN polymarket.trading_processes p ON p.process_id = e.process_id
  WHERE e.experiment_id = NULLIF(:'experiment_id', '')::uuid
)
...
WITH selected AS (
  SELECT e.*, p.config AS process_config
  FROM polymarket.btc_paper_experiments e
  JOIN polymarket.trading_processes p ON p.process_id=e.process_id
  WHERE e.experiment_id=NULLIF(:'experiment_id','')::uuid
), process_orders AS (
...
WITH selected AS (
  SELECT e.*,p.last_error,p.config AS process_config
  FROM polymarket.btc_paper_experiments e
  JOIN polymarket.trading_processes p USING(process_id)
  WHERE e.experiment_id=NULLIF(:'experiment_id','')::uuid
  ...
  SELECT
    (SELECT count(*) FROM public.migrations
      WHERE timestamp=1777123000000
        AND name='AddBtcPhase6PaperCapital1777123000000') AS phase6_migrations,
    ...
    (SELECT count(*) FROM expected_constraints e JOIN pg_namespace n ON n.nspname='polymarket'
      ...
      AS phase6_constraints,
    (SELECT count(*) FROM expected_indexes e JOIN pg_indexes i
      ON i.schemaname='polymarket' AND i.indexname=e.index_name) AS phase6_indexes,
  ...
  SELECT ss.phase6_migrations,ss.ledger_tables,ss.phase6_constraints,ss.phase6_indexes,
...
SELECT *,coalesce((phase6_migrations=1 AND ledger_tables=1 AND phase6_constraints=15
    AND phase6_indexes=5 AND one_day_compression_policies=4
...
SELECT :'readiness_phase6_migrations'::bigint AS phase6_migrations,
  :'readiness_phase6_constraints'::bigint AS phase6_constraints,
  :'readiness_phase6_indexes'::bigint AS phase6_indexes,
...
\echo '== ML shadow diagnostics (never a deterministic Phase 6 profitability gate) =='
...
WITH selected AS (
  SELECT e.*,
         (p.config #>> '{raw,strategy,min_net_edge_per_share}')::numeric AS min_edge_per_share,
         (p.config #>> '{raw,strategy,min_net_edge_usd}')::numeric AS min_edge_usd
  FROM polymarket.btc_paper_experiments e
  JOIN polymarket.trading_processes p USING(process_id)
  WHERE e.experiment_id=NULLIF(:'experiment_id','')::uuid
), opportunities AS (
...
WITH selected AS (
  ...
  SELECT * FROM (VALUES
    ('latency_300ms_depth_65pct'::text,300::numeric,0.65::numeric),
    ('latency_600ms_depth_50pct'::text,600::numeric,0.50::numeric)
  ) AS v(scenario_key,arrival_latency_ms,visible_depth_haircut)
), eligible AS (
...
WITH selected AS (
  ...
  SELECT 'process_raw_config' AS source,item
  FROM selected s
  ...
  UNION ALL
  SELECT 'experiment_frozen_config',item
  FROM selected s
  CROSS JOIN LATERAL jsonb_array_elements(CASE
    WHEN jsonb_typeof(coalesce(s.summary->'paper_stress_previews',s.config->'paper_stress_previews'))='array'
      THEN coalesce(s.summary->'paper_stress_previews',s.config->'paper_stress_previews')
      ELSE '[]'::jsonb END) a(item)
), definition_check AS (
  SELECT s.scenario_key,src.source,count(d.item) AS matching_definitions
  FROM expected_scenarios s
  CROSS JOIN (VALUES('process_raw_config'::text),('experiment_frozen_config'::text)) src(source)
  LEFT JOIN definition_sources d ON d.source=src.source
    AND d.item->>'scenario_key'=s.scenario_key
    AND (
  ...
  GROUP BY s.scenario_key,src.source
), eligible AS (
  ...
  SELECT count(*)=4 AND bool_and(matching_definitions=1) AS pass FROM definition_check
)
...
WITH selected AS (
  SELECT e.*,(p.config #>> '{raw,strategy,min_net_edge_per_share}')::numeric AS min_edge_per_share,
         (p.config #>> '{raw,strategy,min_net_edge_usd}')::numeric AS min_edge_usd
  FROM polymarket.btc_paper_experiments e JOIN polymarket.trading_processes p USING(process_id)
  WHERE e.experiment_id=NULLIF(:'experiment_id','')::uuid
), trades AS (
...
\echo '== Preregistered Phase 6 classification =='
SELECT :'identity_experiment_id'::uuid AS experiment_id,
  ...
  CASE
    ...
    ELSE 'EDGE_SUPPORTED_FOR_PHASE8_PLANNING'
  END AS phase6_classification,
  CASE
    WHEN :'identity_experiment_status'='failed'
      OR NOT :'identity_identity_gate_pass'::boolean
      OR NOT :'integrity_gate_pass'::boolean
    ...
    ELSE 'All preregistered Phase 6 gates pass; this supports Phase 8 planning only and does not authorize live capital.'
...
SELECT :'final_experiment_id'::uuid AS experiment_id,
  ...
  :'final_phase6_classification' AS phase6_classification,
  :'final_classification_reason' AS classification_reason;
\echo PHASE6_CLASSIFICATION= :final_phase6_classification
