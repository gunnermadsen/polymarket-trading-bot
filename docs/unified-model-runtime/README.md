...
RTDS midpoint history and candle construction have one shared owner: the [RTDS repository](rtds-repository.md). Every shared runtime restores it at startup from persisted ticks, independently of process startup order, and keeps it current through the existing gRPC subscription. Consumers use its read-only causal API; no process-specific seed or candle builder is permitted. Existing candle/readiness instrumentation is preserved. RTDS midpoint candles must not replace canonical Chainlink OHLC for an adapter trained on a different product.
