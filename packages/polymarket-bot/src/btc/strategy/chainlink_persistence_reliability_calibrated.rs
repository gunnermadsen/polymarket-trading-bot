...
pub(super) const STRATEGY_VERSION: &str =
    "btc_5m_chainlink_persistence_reliability_calibrated_fair_value_research_v1";
pub(super) const FEATURE_SCHEMA_VERSION: &str = "btc_5m_features_v3";
pub(super) const PROFILE_ID: &str =
    "btc5m-chainlink-persistence-reliability-calibrated-20260725-v1";
pub(super) const PROFILE_SHA256: &str =
    "b60bc06acded49b8525c5ef05530c56e0c35871f460fe24ccda9ede89cb9e75e";

const PROFILE_ARTIFACT: &str =
    include_str!("profiles/btc5m-chainlink-persistence-reliability-calibrated-20260725-v1.json");
const PROFILE_STATUS: &str = "research_only";
const ESTIMATOR: &str = "chainlink_persistence_reliability_calibrated_fair_value_v1";
const PARAMETER_SOURCE: &str = "offline_market_weighted_conditional_logit_calibration_20260725";
const CALIBRATION: &str = "conditional_positive_logit_slope_offline_v1";
const UNCERTAINTY_POLICY: &str = "existing_btc_v1_plus_reliability_research_heuristic";

static COMPILED_PROFILE: OnceLock<
    Result<ChainlinkPersistenceReliabilityCalibratedProfile, BtcRejectReason>,
> = OnceLock::new();
...
pub(super) struct ChainlinkPersistenceReliabilityCalibratedProfile {
    profile_id: String,
    status: String,
...
struct CalibrationParameters {
    directional_intercept: Decimal,
    neutral_slope: Decimal,
    slope_intercept_adjustment: Decimal,
    reliability_weight: Decimal,
    confirmed_quality_weight: Decimal,
    confirmation_deficiency_penalty: Decimal,
    volatility_excess_penalty: Decimal,
    late_window_penalty: Decimal,
    min_slope: Decimal,
    max_slope: Decimal,
...
struct ResearchEvidence {
    ...
    total_markets: u32,
    total_samples: u32,
    training_markets: u32,
    validation_markets: u32,
    holdout_markets: u32,
    regularization: Decimal,
    holdout_baseline_log_loss: Decimal,
    holdout_calibrated_log_loss: Decimal,
...
) -> Result<&'static ChainlinkPersistenceReliabilityCalibratedProfile, BtcRejectReason> {
...
pub(super) fn estimate(
    ...
    profile: &ChainlinkPersistenceReliabilityCalibratedProfile,
    ...
    let up_probability = calibrate_probability(
...
fn calibrate_probability(
    probability: Decimal,
    evidence_reliability: Decimal,
    confirmed_strength: Decimal,
    ...
    let neutral_slope = calibration
        .neutral_slope
        .to_f64()
        .ok_or(BtcRejectReason::InvalidFeatureValue)?;
    let theta = neutral_slope.exp_m1().ln()
        + calibration
            .slope_intercept_adjustment
            .to_f64()
            .ok_or(BtcRejectReason::InvalidFeatureValue)?
        + calibration
            .reliability_weight
        ...
        + calibration
            ...
            * (confirmed_strength
                ...
                .clamp(0.0, 1.0)
        - calibration
            .confirmation_deficiency_penalty
            .to_f64()
            .ok_or(BtcRejectReason::InvalidFeatureValue)?
            * (confirmation_deficiency
                .to_f64()
    ...
    let max_abs_logit = calibration
        .max_abs_logit
        .to_f64()
        .ok_or(BtcRejectReason::InvalidFeatureValue)?;
    let raw_logit = (probability_f64 / (1.0 - probability_f64))
        .ln()
        .clamp(-max_abs_logit, max_abs_logit);
    let directional_intercept = calibration
        .directional_intercept
        .to_f64()
        .ok_or(BtcRejectReason::InvalidFeatureValue)?;
    let calibrated =
        decimal_from_f64(1.0 / (1.0 + (-(directional_intercept + slope * raw_logit)).exp()))?;
    Ok(calibrated.clamp(
        profile.probability_floor,
        Decimal::ONE - profile.probability_floor,
    ))
...
fn decode_and_validate_profile(
    ...
    let profile: ChainlinkPersistenceReliabilityCalibratedProfile =
    ...
    let valid = profile.profile_id == PROFILE_ID
        ...
        && profile.calibration_parameters
            == (CalibrationParameters {
                directional_intercept: Decimal::ZERO,
                neutral_slope: dec!(0.85),
                slope_intercept_adjustment: dec!(0.5725050613727726),
                reliability_weight: Decimal::ZERO,
                confirmed_quality_weight: Decimal::ZERO,
                confirmation_deficiency_penalty: dec!(0.1643625702655827),
                volatility_excess_penalty: dec!(0.0009197638088653648),
                late_window_penalty: Decimal::ZERO,
                min_slope: dec!(0.35),
                max_slope: dec!(1.35),
        ...
        && profile.research_evidence
            == (ResearchEvidence {
                source_process_id: "5635682b-4d79-4786-9462-668b2618a415".to_string(),
                source_strategy_version:
                    "btc_5m_chainlink_persistence_calibrated_fair_value_research_v1".to_string(),
                source_profile_id: "btc5m-chainlink-persistence-calibrated-20260720-v1".to_string(),
                source_profile_sha256:
                    "238e7ed1b3676a3da61be3b3c8f9d48d7d05cad92e158b921846b9bfe495c1a3".to_string(),
                dataset_sha256: "e2bf7c7f499caef5a3d8a323b2d42a38470392d9a301d938dd8a991a011a8a6b"
                    .to_string(),
                first_window_start: "2026-07-20T15:05:00Z".to_string(),
                last_window_start: "2026-07-25T19:55:00Z".to_string(),
                total_markets: 930,
                total_samples: 2131,
                training_markets: 558,
                validation_markets: 186,
                holdout_markets: 186,
                regularization: dec!(0.001),
                holdout_baseline_log_loss: dec!(0.5972787234986193),
                holdout_calibrated_log_loss: dec!(0.5811731567831319),
                holdout_baseline_brier_score: dec!(0.2055704891317407),
                holdout_calibrated_brier_score: dec!(0.19918038534935928),
...
mod tests {
    ...
    fn profile() -> &'static ChainlinkPersistenceReliabilityCalibratedProfile {
    ...
    #[test]
    fn conditional_calibration_preserves_direction_symmetry_and_monotonicity() {
        let profile = profile();
        let inputs = (dec!(0.82), dec!(0.60), dec!(0.20), dec!(0.15), dec!(0.30));
        ...
        .unwrap();
        let down_mirror = calibrate_probability(
            dec!(0.30),
            inputs.0,
            inputs.1,
            inputs.2,
        ...
        .unwrap();
        let stronger = calibrate_probability(
            dec!(0.80),
            inputs.0,
            inputs.1,
            inputs.2,
        ...
        assert!((up + down_mirror - Decimal::ONE).abs() < dec!(0.000000000000001));
        assert!(stronger > up);
        assert!(up > dec!(0.5));
        assert!(down_mirror < dec!(0.5));
    }

    #[test]
    fn neutral_conditional_parameters_reproduce_original_fixed_shrink() {
        let mut neutral = profile().clone();
        neutral.calibration_parameters.slope_intercept_adjustment = Decimal::ZERO;
        neutral.calibration_parameters.reliability_weight = Decimal::ZERO;
        neutral.calibration_parameters.confirmed_quality_weight = Decimal::ZERO;
        neutral
            .calibration_parameters
            .confirmation_deficiency_penalty = Decimal::ZERO;
        neutral.calibration_parameters.volatility_excess_penalty = Decimal::ZERO;
        neutral.calibration_parameters.late_window_penalty = Decimal::ZERO;

        let calibrated = calibrate_probability(
            dec!(0.70),
            Decimal::ONE,
            Decimal::ONE,
            Decimal::ZERO,
        ...
        .unwrap();
        let expected = logistic(dec!(0.85) * dec!(0.8472978603872036137101075065)).unwrap();
        assert!((calibrated - expected).abs() < dec!(0.000000000000001));
    }
}
