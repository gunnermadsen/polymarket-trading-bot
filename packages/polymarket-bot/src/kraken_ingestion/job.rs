...
impl KrakenDataset {
    pub const ALL: [Self; 16] = [
        ...
        Self::OpenInterest,
        Self::FutureBasis,
        Self::AggressorDifferential,
        Self::TradeVolume,
        Self::TradeCount,
        Self::Cvd,
        Self::LiquidationVolume,
        Self::Spreads,
        Self::Liquidity,
        Self::Slippage,
        Self::FundingRates,
    ...
    pub const fn is_candle(self) -> bool {
        matches!(
            self,
            Self::TradeCandles | Self::MarkCandles | Self::SpotCandles
        )
    }
...
pub struct KrakenBackfillJob {
    ...
    pub expected_work_units: i64,
    pub processed_work_units: i64,
}
...
#[derive(Debug, Clone)]
pub struct ClaimedKrakenJob {
    pub job: KrakenBackfillJob,
...
impl NormalizedRows {
    pub fn len(&self) -> usize {
        match self {
            Self::Instruments(rows) => rows.len(),
            Self::FeeSchedules(rows) => rows.len(),
            Self::Candles(rows) => rows.len(),
            Self::Analytics(rows) => rows.len(),
            Self::FundingRates(rows) => rows.len(),
        }
    }
