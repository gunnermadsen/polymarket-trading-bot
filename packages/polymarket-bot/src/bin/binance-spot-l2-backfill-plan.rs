...
use polymarket_bot::{
    ...
    ingestion::{
        cryptohft_binance_l2::{
            day_artifact_logical_key_for_market, validate_representative_day_quality_for_market,
            CryptoHftBinanceMarket, BINANCE_SPOT_L2_MATERIALIZATION_CONTRACT,
        },
...
const RETRY_GENERATION_ENV: &str = "POLYMARKET_BINANCE_SPOT_L2_RETRY_GENERATION";
...
async fn main() -> Result<()> {
    ...
    let pool = PgPoolOptions::new()
        ...
        .await
        .context("failed to connect Binance spot L2 planner to PostgreSQL")?;
    ...
    let representative_key =
        day_artifact_logical_key_for_market(CryptoHftBinanceMarket::Spot, start);
    ...
    .fetch_optional(&pool)
    .await
    .context("failed to inspect the representative Binance spot L2 source audit")?;
    match representative {
        Some((status, metadata)) if status == "completed" => {
            validate_representative_day_quality_for_market(
                CryptoHftBinanceMarket::Spot,
                &metadata,
            )
            .context(
                "representative CryptoHFT spot source audit is not causally classified; refusing to enqueue the full Binance spot L2 backfill",
            )?;
        }
        _ => {
            let request = daily_request(start, retry_generation)?;
            let job = repository.enqueue(&request).await.context(
                "failed to enqueue or recover the representative Binance spot L2 source audit",
            )?;
            match job.status {
                ...
                | BackfillJobStatus::CancelRequested => {
                    info!(
                        ...
                        retry_generation,
                        status = ?job.status,
                        "representative Binance spot L2 source audit active; full plan remains gated"
                    );
                    return Ok(());
                }
    ...
    let mut inventory_entries = 0u64;
    for date in shard_dates {
        let request = daily_request(date, retry_generation)?;
        let job = repository
            .enqueue(&request)
            .await
            .with_context(|| format!("failed to enqueue Binance spot L2 shard for {date}"))?;
        info!(job_id = %job.job_id, shard_date = %date, status = ?job.status, "Binance spot L2 shard ready");
        inventory_entries = inventory_entries.saturating_add(1);
    }

    info!(%start, %end, inventory_entries, retry_generation, "Binance spot L2 historical inventory ready");
    Ok(())
}
...
fn daily_request(
    ...
    if retry_generation == 0 {
        bail!("Binance spot L2 retry generation must be positive");
    }
    ...
    BackfillRequest {
        ...
        idempotency_key: format!(
            "binance-spot-btcusdt-l2:{date}:{BINANCE_SPOT_L2_MATERIALIZATION_CONTRACT}:retry-{retry_generation}"
        ),
    }
    .validate()
    .with_context(|| format!("invalid Binance spot L2 shard request for {date}"))
}
...
fn approved_shard_dates() -> Result<(NaiveDate, NaiveDate, Vec<NaiveDate>)> {
    ...
    if end <= start {
        bail!("Binance spot L2 backfill end must be later than start");
    }
    ...
    if dates.len() != EXPECTED_DAILY_SHARDS {
        bail!(
            "Binance spot L2 approved range must contain exactly {EXPECTED_DAILY_SHARDS} UTC-day shards; found {}",
            dates.len()
        );
    }
...
mod tests {
    ...
    #[test]
    fn daily_request_identity_is_spot_specific() {
        ...
        assert_eq!(
            ...
            format!(
                "binance-spot-btcusdt-l2:{date}:{BINANCE_SPOT_L2_MATERIALIZATION_CONTRACT}:retry-1"
            )
        );
        ...
        assert_eq!(
            ...
            format!(
                "cryptohftdata:binance-spot:BTCUSDT:l2-day:{BINANCE_SPOT_L2_MATERIALIZATION_CONTRACT}:{date}"
            )
        );
    }
