...
eligible_markets AS MATERIALIZED (
  SELECT
    ...
    COALESCE(fact.opening_boundary, market.reference_price::double precision)
      AS opening_boundary,
...
canonical_klines AS MATERIALIZED (
  SELECT
    ...
    kline.taker_buy_base_volume,
    kline.taker_buy_quote_volume,
    greatest(kline.received_at, kline.provider_available_at) AS available_at,
    'market_data.binance_spot_btcusdt_one_second_ohlcv'::text
      AS source_relation,
    kline.capture_artifact_id::text AS source_artifact_id,
    2 AS source_priority
  FROM market_data.binance_spot_btcusdt_one_second_ohlcv kline
  WHERE kline.source = 'binance_spot'
    AND kline.symbol = 'BTCUSDT'
    AND kline.open_timestamp >= %(batch_start)s - interval '1 second'
    AND kline.open_timestamp < %(batch_end)s
    AND greatest(kline.received_at, kline.provider_available_at) IS NOT NULL
    AND kline.close_timestamp <= greatest(
      kline.received_at, kline.provider_available_at
    )
),
...
SELECT
  ...
  kline.available_at AS kline_available_at,
  kline.source_relation AS kline_source_relation,
  kline.source_artifact_id AS kline_source_artifact_id
