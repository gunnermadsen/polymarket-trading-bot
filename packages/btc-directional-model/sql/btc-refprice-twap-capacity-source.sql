...
ranked AS MATERIALIZED (
  SELECT
    ...
    artifact.provider,
    row_number() OVER (
      PARTITION BY snapshot.market_id, snapshot.sampled_at
      ORDER BY
        CASE artifact.provider
          WHEN 'polymarket_local_orderbook_capacity_execution_snapshots_v1' THEN 2
          ELSE 1
        END DESC,
  ...
  WHERE snapshot.sampled_at >= %(batch_start)s
    ...
    AND snapshot.schema_version IN (
      'btc5m-capacity-book-1-240s-v2',
...
FROM ranked snapshot
JOIN eligible_markets market USING (market_id)
WHERE snapshot.source_rank = 1
