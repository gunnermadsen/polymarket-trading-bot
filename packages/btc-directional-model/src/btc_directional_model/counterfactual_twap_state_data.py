...
def construct_binance_labels(labels: pl.DataFrame, binance: pl.DataFrame) -> pl.DataFrame:
    ...
    return labels.join(aggregated, on="market_id", how="left", validate="1:1").with_columns(
        (pl.col("binance_close_twap60") / pl.col("binance_open_twap60"))
        .log().mul(10_000.0).alias("binance_raw_margin_bps")
    ).with_columns(
    ...
    *,
    ...
    def block_metrics(start: datetime, end: datetime, target: str) -> dict[str, Any]:
        ...
        return {
            ...
            "agreement": float(
                (admitted["binance_corrected_label_up"] == admitted[target]).mean()
            ),
            "margin_mae_bps": float(
                (admitted["binance_corrected_margin_bps"] - admitted["proxy_margin_bps"])
                .abs().mean()
            ) if target == "proxy_label_up" else None,
        }

    report = {
        "target": "five_minute_twap_margin_residual_bps",
        "features": list(features),
        "fit": {"start": overlap_start.isoformat(), "end": fit_end.isoformat(), "markets": fit.height},
        "validation": block_metrics(fit_end, validation_end, "proxy_label_up"),
        "authentic_confirmation": block_metrics(validation_end, authentic_end, "authentic_label_up"),
...
def assign_priority_labels(
    ...
    *,
    ...
    proxy_weight = (
        pl.when(abs_proxy < CHAINLINK_UNCERTAINTY_BPS).then(0.0)
        .when(abs_proxy < 2 * CHAINLINK_UNCERTAINTY_BPS).then(0.25)
        .when(abs_proxy < 3 * CHAINLINK_UNCERTAINTY_BPS).then(0.50)
        .otherwise(0.75)
    )
    abs_binance = pl.col("binance_corrected_margin_bps").abs()
    binance_weight = (
        pl.when(abs_binance < 1.0).then(0.0)
        .when(abs_binance < 2.0).then(0.25)
        .otherwise(0.75)
    )
    ...
    assigned = joined.with_columns(
        pl.when(valid_authentic).then(pl.col("authentic_label_up"))
        .when(valid_proxy).then(pl.col("proxy_label_up"))
        .when(valid_binance).then(pl.col("binance_corrected_label_up"))
        .otherwise(None).cast(pl.Int8).alias("label_up"),
        pl.when(valid_authentic).then(1.0)
        .when(valid_proxy).then(proxy_weight)
        .when(valid_binance).then(binance_weight)
        .otherwise(0.0).alias("base_label_weight"),
        ...
        .when(valid_proxy).then(pl.lit("chainlink_reconstructed_twap60"))
        .when(valid_binance).then(pl.lit("binance_synthetic_twap60"))
        .otherwise(pl.lit("excluded")).alias("label_source"),
        pl.when(valid_authentic).then(pl.col("authentic_margin_bps"))
        .when(valid_proxy).then(pl.col("proxy_margin_bps"))
        .when(valid_binance).then(pl.col("binance_corrected_margin_bps"))
        .otherwise(None).alias("target_margin_bps"),
        pl.when(valid_authentic).then(pl.col("twap_open_price"))
        .when(valid_proxy).then(pl.col("proxy_open_price"))
        .when(valid_binance).then(pl.col("binance_open_twap60"))
        .otherwise(None).alias("opening_twap60"),
    )
...
def _build_core_features(
    ...
    complete = frame.group_by("market_id").agg(
        pl.len().alias("rows"),
        ...
        (pl.col("rows") == 300) & (pl.col("seconds") == 300)
        & (pl.col("minimum") == 0) & (pl.col("maximum") == 299)
    ).select("market_id")
    ...
    return frame.filter(
        ...
        "oracle_block_timestamp", "oracle_phase_id", "oracle_round_id", "oracle_block_number",
        "oracle_log_index", "oracle_window_open_price", "oracle_round_changed", strict=False,
    )


def _rolling_mean_at(times: np.ndarray, values: np.ndarray, points: np.ndarray, seconds: int) -> np.ndarray:
    if len(times) == 0:
...
def attach_causal_twap_features(
    ...
    chain = canonical_refprice_path(refprice).sort("provider_available_at").filter(
        pl.col("source_timestamp") == pl.col("source_timestamp").cum_max()
    )
    ...
    for window in (30, 60):
        for horizon in (5, 15, 30, 60):
            prior = _rolling_mean_at(chain_times, chain_values, points - np.timedelta64(horizon, "s"), window)
            current = chain30 if window == 30 else chain60
            chain_slopes[(window, horizon)] = np.log(current / prior) * 10_000.0

    binance_by_market = {key[0]: part for key, part in binance.partition_by("market_id", as_dict=True).items()}
    ...
    for horizon in (5, 15, 30, 60):
        result = result.with_columns(
            ...
            pl.Series(f"twap60_slope_{horizon}s_bps", np.where(use_chain, chain_slopes[(60, horizon)], b_slopes[(60, horizon)])),
        )
    result = result.with_columns(
        pl.Series("chainlink_twap30", chain30), pl.Series("chainlink_twap60", chain60),
        pl.Series("binance_twap30", b30), pl.Series("binance_twap60", b60),
        pl.Series("current_twap30", twap30), pl.Series("current_twap60", twap60),
        ...
        .log().mul(10_000).alias("twap60_change_from_open_bps"),
        (pl.col("current_twap30") / pl.col("opening_twap60"))
        .log().mul(10_000).alias("twap30_gap_to_opening_twap60_bps"),
        (pl.col("current_twap60") / pl.col("opening_twap60"))
        .log().mul(10_000).alias("twap60_gap_to_opening_twap60_bps"),
        (pl.col("current_twap30") / pl.col("current_twap60"))
        .log().mul(10_000).alias("twap30_minus_twap60_bps"),
        (pl.col("chainlink_twap30") / pl.col("binance_twap30"))
        .log().mul(-10_000).alias("binance_twap30_minus_chainlink_twap30_bps"),
        ...
        (pl.col("chainlink_ref_boundary_gap_bps") - pl.col("twap60_gap_to_opening_twap60_bps"))
        .alias("refprice_minus_twap60_bps"),
        (pl.col("twap30_change_from_open_bps").sign() * pl.col("twap60_change_from_open_bps").sign())
        .alias("twap_directional_agreement"),
        (pl.col("chainlink_ref_boundary_gap_bps").sign() * pl.col("twap60_change_from_open_bps").sign())
        .alias("refprice_twap_directional_agreement"),
        (pl.col("twap30_slope_5s_bps") - pl.col("twap30_slope_15s_bps") / 3.0)
        .alias("twap_acceleration_bps"),
        (pl.col("twap30_minus_twap60_bps") - pl.col("twap30_slope_5s_bps") + pl.col("twap60_slope_5s_bps"))
        .alias("twap_convergence_velocity_bps"),
        (pl.col("chainlink_ref_return_5s_bps") - pl.col("twap30_slope_5s_bps"))
        .alias("refprice_pressure_bps"),
        pl.col("btc_boundary_cross_count").cast(pl.Float64).alias("twap_boundary_cross_count"),
        pl.col("btc_seconds_since_boundary_cross").cast(pl.Float64).alias("twap_seconds_since_boundary_cross"),
        ...
        pl.col("chainlink_ref_realized_volatility_60s_bps").alias("twap_realized_volatility_bps"),
        (pl.col("twap60_gap_to_opening_twap60_bps") / pl.max_horizontal(
...
def build_counterfactual_frame(
    ...
    *,
    ...
    canonical = canonical_refprice_path(refprice).sort(convention.time_column).unique(
        subset=[convention.time_column], keep="last"
    ...
    core = _build_core_features(
        load_source_group(paths.base, "core_current"), boundaries, load_source_group(paths.base, "oracle")
    )
    ...
    execution = load_source_group(paths.base, "execution").filter(pl.col("window_start") >= official_start)
    frame = attach_execution(frame, execution)
    frame = attach_candle_context(frame, load_source_group(paths.base, "candles").unique(subset=["close_timestamp"], keep="last"))
