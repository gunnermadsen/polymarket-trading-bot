...
fn filesystem_capacity(path: &Path) -> Result<(u64, u64)> {
    let output = std::process::Command::new("df")
        .args([
            "-Pk",
            path.to_str()
                .context("CryptoHFT archive path was not UTF-8")?,
        ])
        .output()
        .context("failed to inspect CryptoHFT archive filesystem")?;
    if !output.status.success() {
        bail!("df rejected the CryptoHFT archive filesystem query");
    }
    let line = std::str::from_utf8(&output.stdout)
        .context("df output was not UTF-8")?
        .lines()
        .last()
        .context("df output did not include filesystem capacity")?;
    let fields = line.split_whitespace().collect::<Vec<_>>();
    if fields.len() < 4 {
        bail!("df output did not include filesystem capacity fields");
    }
    let total_kib = fields[1]
        .parse::<u64>()
        .context("invalid df total capacity")?;
    let available_kib = fields[3]
        .parse::<u64>()
        .context("invalid df available capacity")?;
    Ok((
...
async fn wait_for_request_slot(pool: &PgPool, cancellation: &ArchiveCancellation) -> Result<()> {
    let scheduled_at = reserve_cryptohft_request_slot(pool).await?;
    loop {
        check_cancelled(cancellation)?;
...
async fn reserve_cryptohft_request_slot(pool: &PgPool) -> Result<DateTime<Utc>> {
    let mut tx = pool
        .begin()
        .await
        .context("failed to begin CryptoHFT request-budget transaction")?;
    let row = sqlx::query(
        "SELECT GREATEST(next_request_at,clock_timestamp()) AS scheduled_at,spacing_milliseconds FROM polymarket.cryptohft_request_budget WHERE provider='cryptohftdata' FOR UPDATE",
    )
    .fetch_one(&mut *tx)
    .await
    ...
    sqlx::query("UPDATE polymarket.cryptohft_request_budget SET next_request_at=$1+($2*interval '1 millisecond'),updated_at=now() WHERE provider='cryptohftdata'")
        .bind(scheduled_at)
