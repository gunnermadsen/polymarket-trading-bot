...
pub fn validate_empty_request(
    ...
    if request.strategy_key != descriptor.strategy_key.as_ref() {
        return Err(BackfillExecutionError::invalid(
            ...
            "request strategy key does not match the Binance backfill strategy",
        ));
    }
        ...
        .is_some_and(|value| value.is_empty())
    {
        return Err(BackfillExecutionError::invalid(
            "parameters_invalid",
            "this Binance backfill strategy accepts no parameters",
        ));
    }
...
pub fn daily_shards(
    ...
    while cursor < request.range_end {
        let next_midnight = cursor
            ...
            .ok_or_else(|| {
                BackfillExecutionError::invalid("range_overflow", "Binance shard range overflowed")
            })?;
