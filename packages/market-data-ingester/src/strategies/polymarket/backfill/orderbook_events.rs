...
pub struct PolymarketBtcOrderbookEventsBackfill(BackfillSupport);

impl PolymarketBtcOrderbookEventsBackfill {
    pub fn new() -> Result<Self, BackfillExecutionError> {
        BackfillSupport::new(Kind::OrderbookEvents).map(Self)
    }
}

#[async_trait]
impl BackfillWorkerStrategy for PolymarketBtcOrderbookEventsBackfill {
    ...
    async fn execute_backfill(
        ...
        self.0.execute_orderbook_events(&context, &shard).await
    }
}
