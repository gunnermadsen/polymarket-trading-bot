...
def rule(uid,title,expr,wait='2m',severity='warning',description=''):
 return dict(uid=uid,title=title,condition='C',data=[
  ...
  noDataState='OK',execErrState='Error',**{'for':wait},annotations=dict(summary=title,description=description,dashboardUid='unified-model-runtime'),labels=dict(family='umr',service='polymarket-bot',severity=severity),isPaused=False)
rules=[
