...
row('Orderbook observation diagnostics')
panel('Observation book failure causes','sum by(process_id,reason)(rate(polymarket_umr_book_input_failures_total{'+S+'}[$__rate_interval]))','Book selection failures per side at the immutable observation boundary. Distinguishes timestamp, identity, epoch and integrity failures; these counts are not model admission rejections.','ops',x=0,w=24,legend='{{model_key}} · {{reason}}');Y+=8
row('Investigation')
